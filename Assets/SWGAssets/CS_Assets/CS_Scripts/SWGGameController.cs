...
namespace ScrambledWordGame
{
    ...
    /// </summary>
	public class SWGGameController : FGProgram
    {
		...
		public void timeup() {
			...
			timeLeft = -1.0f;
			prefinish = true;
			screenFController.wordGameResult = WordGameResult.timeup;
		}

		bool finished = false;
		bool prefinish = false;
		float elapsedTime = 0.0f;
		public ScreenFController screenFController;
		...
		WordWithHints theWordWithHints;
        ...
        public Transform letterCurrent;
		// esto era internal
        ...
        [Tooltip("The number of letters added to the game in each level")]
        public int lettersIncrease = 1;
		...
		public void go (string word) 
        {
			...
			newWord.word = word;
			newWord.textHint = "";
			newWord.imageHint = null;
			theWordWithHints = newWord;
			time = 25; // reset time
			finished = false;
			isSwapping = false;
			prefinish = false;
...
//                    if (wordsWithHints[index].word.Contains(" "))
//                    {
//                        wordsInSentence = 1;
//
...
//                }
                ...
                if ( wordFound == true )
                {
					...
					for ( int letterIndex = 0; letterIndex < theWordWithHints.word.Length; letterIndex++ )
                    {
                        // Create a new letter
                        ...
                        if (wordsInSentence > 1)
                        {
							...
							while ( letterIndex < theWordWithHints.word.Length && theWordWithHints.word[letterIndex].ToString() != " " )
                            {
								newLetter.Find("Text").GetComponent<Text>().text += theWordWithHints.word[letterIndex].ToString();
					...
					if ( wordsInSentence <= 1 ) currentWord = theWordWithHints.word;
                    ...
                    if  ( textHintObject )
                    {
						...
						if ( theWordWithHints.textHint != null && theWordWithHints.textHint != string.Empty )
                        {
							...
							textHintObject.text = theWordWithHints.textHint;
                    ...
                    if ( imageHintObject )
                    {
						...
						if ( theWordWithHints.imageHint )
                        {
							...
							imageHintObject.sprite = theWordWithHints.imageHint;
            ...
            if ( wordFound == false )
            {
                ...
                //    if ( wordFoundIndex >= wordsWithHints.Length - 1 ) 
                //    {
                //        // Stop the timer
                //        timeRunning = false;
                ...
                //    }
                //    else if ( currentWord.Length < longestWordLength) // Otherwise, if we didn't find any words with the required letters count, try to move up to a higher letters count and look again
                //    {
        ...
        IEnumerator CheckWord()
        {
            ...
            if (tempWord == currentWord)
            {
				...
				prefinish = true;
				screenFController.wordGameResult = WordGameResult.wordOK;
...
//                }
        ...
        public void ShowHint()
        {
            ...
            // Show the text hint
			if ( textHintObject && theWordWithHints.textHint != string.Empty  ) textHintObject.gameObject.SetActive(true);

            // Show the image hint
			if ( imageHintObject && theWordWithHints.imageHint ) imageHintObject.gameObject.SetActive(true);
        }
